 ...
 * [구현] 체이닝 방식 해시맵.
 *
 * 참고: 이 폴더에 Map.java 가 없다. 인터페이스는 src/main 에서 온다.
 *
...
public class ChainingHashMap<K, V> implements Map<K, V> {
    ...
    Node<K, V> findNode(Object key) {
        ...
        for (Node<K, V> n = buckets[bucket]; n != null; n = n.next) {
            if (n.key.equals(key)) {      // 같은 버킷이라고 같은 키가 아니다
                return n;
            }
        }
        return null;
    ...
    public V put(K key, V value) {
        ...
        if (existing != null) {
            ...
            existing.value = value;       // 새로 매달면 같은 키가 두 개가 된다
            afterPut(key, false);
            return old;
        }

        int bucket = bucketOf(key, buckets.length);
        buckets[bucket] = new Node<>(key, value, buckets[bucket]);   // 앞에 매다는 게 O(1)
        size++;
        afterPut(key, true);
     ...
     *
     * 생각할 것
     *   - 버킷 수가 바뀌면 같은 키의 버킷 번호도 바뀐다. 그래서 옮기는 게 아니라 다시 계산해야 한다.
    ...
    public V remove(Object key) {
        ...
        int bucket = bucketOf(key, buckets.length);
        Node<K, V> prev = null;
        for (Node<K, V> n = buckets[bucket]; n != null; prev = n, n = n.next) {
            if (n.key.equals(key)) {
                if (prev == null) {
                    buckets[bucket] = n.next;    // 사슬의 첫 노드였다
                } else {
                    prev.next = n.next;
                }
                n.next = null;
