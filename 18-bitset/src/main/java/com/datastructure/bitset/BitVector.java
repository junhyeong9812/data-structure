 ...
 *
 * 자바에서 `boolean[]` 은 원소 하나가 **1비트가 아니라 1바이트**다.
 * JVM 이 바이트 단위로 주소를 잡기 때문이다. 100만 개면 1MB 다.
 * 비트로 눌러 담으면 125KB 다. **8배 차이**이고, 그것만으로도 이유가 된다.
 *
 * 그런데 진짜 이유는 메모리가 아니다.
 *
 * **집합 연산이 64배 빨라진다.**
 ...
 * 여기서는 **고정 크기**로 둔다. 비트맵 인덱스의 실제 쓰임이 그렇고(행 수가 정해져 있다),
...
public interface BitVector {
     ...
     *
     * 이 메서드가 있는 이유는 **켜진 것만 순회**하기 위해서다.
