...
public class KahnResolver implements Resolver {
    ...
    public List<String> resolve() {
        relaxations = 0;
        // TODO 4: 위 javadoc 의 네 단계.
        //
        // 진입 차수는 그래프에서 읽어 **복사해서** 쓴다. 그래프를 고치면 resolve 를 두 번 못 부른다.
        //
        // 다 뽑았는지 확인하는 것을 잊지 마라. 순환이 있으면 조용히 짧은 목록이 나온다.
        // 절반만 설치된 상태가 아무것도 안 한 상태보다 나쁘다. 부분 결과를 주면 안 된다.
        //
        // 꺼낼 것이 여럿일 때 무엇을 먼저 꺼내는지가 답을 정한다. 아무거나 꺼내도 맞지만,
        // 그러면 같은 입력에 같은 답이 나온다는 보장이 없어져 기댓값을 못 쓴다.
        // 무엇을 쓸지는 필드 선언이 이미 정해뒀다.
        //
        // relaxations 는 진입 차수를 내린 횟수다. 간선 수와 같아야 한다.
        throw new UnsupportedOperationException("TODO 4: resolve");
    }
    ...
    public List<List<String>> layers() {
        // TODO 5: resolve 와 같은 방식인데 하나씩이 아니라 **그 시점의 것을 통째로** 꺼낸다.
        //
        // 한 층을 확정한 다음에 그 층이 풀어준 것들로 다음 층을 만든다.
        // 한 개씩 처리하면서 새로 0 이 된 것을 지금 층에 밀어넣으면 층이 뭉개진다.
        // 그래도 순서 자체는 맞아서, 층을 펼쳐놓고 보는 검사로는 안 잡힌다.
        //
        // 여기도 다 놓았는지 확인해야 한다. 순환이면 던진다.
        throw new UnsupportedOperationException("TODO 5: layers");
    }
