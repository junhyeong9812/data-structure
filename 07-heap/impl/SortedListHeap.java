...
/**
 * [구현] 정렬된 배열로 만든 힙 - 가장 먼저 떠오르는 방법.
 *
 * 참고: 이 폴더에 Heap.java 가 없다. 인터페이스는 src/main 에서 온다.
 *
...
public class SortedListHeap<E> implements Heap<E> {
    ...
    public E peek() {
        ...
        return at(size - 1);      // 맨 끝이 가장 앞선 것이다
    }
    ...
    public E poll() {
        if (size == 0) throw new NoSuchElementException("비어 있다");
        // 가장 앞선 것을 배열 끝에 두었으므로 여기서는 밀 필요가 없다.
        // 넣기를 비싸게 하고 꺼내기를 싸게 한 셈이다.
        E value = at(--size);
        elements[size] = null;
        return value;
    }
