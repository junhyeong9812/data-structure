...
/**
 * 좌표 압축. 등장하는 좌표만 0 부터 번호를 다시 매긴다.
 *
...
public final class CoordinateCompressor {
    ...
    /**
     * 구간 목록에 등장하는 좌표를 모아 번호를 매긴다.
     *
     * 생각할 것
     *   - 한 구간이 좌표를 둘 준다. n 개면 후보가 2n 개고, 그중 중복을 걷어낸 것이 답이다.
     *   - 번호는 크기 순서여야 한다. 순서가 어긋나면 압축한 트리의 답이 원본과 달라진다.
     *   - HashSet 에 담았다가 꺼내면 순서가 없다. 정렬이 필요하다.
     *
     * TODO 8: coordinates 를 채워라. intervals 가 null 이면 IllegalArgumentException.
     */
    public CoordinateCompressor(List<Interval> intervals) {
        if (intervals == null) {
            throw new IllegalArgumentException("구간 목록이 null 이다");
        }
        // 빈 목록도 정상 입력이다. 좌표가 0 개인 압축기가 된다.
        throw new UnsupportedOperationException("TODO 8: CoordinateCompressor");
    }
