...
public abstract class ProbeSequenceMap<K, V> implements ProbeMap<K, V> {
    ...
    int indexOf(Object key) {
        ...
        // TODO 1: 탐사 수열을 따라 걸으며 키를 찾아라. 들여다본 칸 수를 lastProbes 에 남겨라.
        //
        //   1. i 를 0 부터 capacity 미만까지 올리며 probe(hash, i) 가 주는 칸을 본다.
        //      i 가 0 일 때가 홈 버킷이다. i 를 capacity 로 제한하는 이유도 생각해보라.
        //      상한이 없으면 가득 찬 테이블에서 안 끝난다.
        //   2. 칸을 볼 때마다 lastProbes 를 하나 올린다. 빈칸을 본 것도 센다.
        //      "없다"를 알려면 그 빈칸을 봐야 했기 때문이다. 이 박스의 측정이 전부 이 수를 센다.
        //   3. EMPTY 를 만나면 없는 것이다. 여기서 멈춘다.
        //   4. TOMBSTONE 은 지나친다. 05번에서 다룬 그대로다. 여기서 멈추면 그 자리를 지나
        //      뒤에 들어간 키를 영영 못 찾는다.
        //   5. OCCUPIED 이고 키가 같으면 그 칸이다.
        //
        // 3번과 4번을 뒤바꾸는 것이 이 자료구조에서 제일 흔한 사고다.
    ...
    public V put(K key, V value) {
        ...
        lastProbes = 0;
        // TODO 2: 같은 수열을 따라 걸으며 넣을 자리를 잡아라. insertAt 이 자리에 앉히는 일을 한다.
        //
        //   1. TODO 1 과 같은 모양으로 걷는다. lastProbes 도 같은 규칙으로 센다.
        //   2. 이미 있는 키를 만나면 값만 바꾸고 이전 값을 반환한다.
        //      새 자리를 잡으면 같은 키가 두 개가 된다.
        //   3. EMPTY 를 만나면 거기가 끝이다. 여기까지 오는 동안 같은 키가 없었으므로 넣어도 된다.
        //   4. TOMBSTONE 자리는 재사용할 수 있다. 다만 만나자마자 넣으면 안 된다.
        //      그 뒤에 같은 키가 이미 있을 수 있기 때문이다(05번에서 잡았던 그 중복이다).
        //      자리만 기억해두고 탐사를 계속한 다음, EMPTY 를 만났을 때 기억해둔 자리에 넣는다.
        //   5. used 는 EMPTY 를 새로 쓸 때만 는다. tombstone 재사용은 이미 센 자리다.
        //      리사이즈 판단이 used 로 이뤄지므로 여기를 틀리면 배열만 커진다.
        //   6. 한 바퀴를 다 돌았는데 EMPTY 가 없었다면? 기억해둔 tombstone 이 있으면 거기 넣고,
        //      그것도 없으면 IllegalStateException 이다. 메시지에 "빈칸"이라는 말을 넣어라.
        //      선형과 이중 해싱은 여기 오지 않는다. 수열이 모든 칸을 방문하기 때문이다.
        //      이차 탐사는 온다. 그게 이 박스의 측정 하나다(QuadraticProbeMapTest).
        //
        // 6번을 "그냥 리사이즈한다"로 때우지 마라. 부하율 0.19 에서도 못 넣는 경우가 있고,
        // 그것을 보여주는 것이 이 문제의 절반이다.
