...
public class RobinHoodMap<K, V> implements ProbeMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        //      이때 걸어온 거리도 쫓겨난 놈의 거리로 바꿔야 한다. 이제 그를 대신해 걷는 것이다.
        //   5. 한 번이라도 뺏었다면 그 뒤로는 3번을 하면 안 된다.
        //      뺏은 순간 원래 키가 테이블에 없다는 것이 확정됐고(있었다면 그 전에 만났다),
        //      지금 들고 있는 것은 테이블에 있던 항목이라 중복 검사가 의미 없기 때문이다.
        //
        // 4번의 부등호가 이 구현의 전부다. <= 로 쓰면 거리가 같은데도 자리를 뺏어
        // 뜻 없는 이동만 늘어난다. 5번을 빼먹으면 같은 키가 두 개 생긴다.
        // 정렬된 배열에 끼워 넣기(01번)와 같은 모양인데, 여기서는 밀어내는 것이 아니라
        // 하나만 들고 자리를 바꿔 앉힌다는 점이 다르다.
