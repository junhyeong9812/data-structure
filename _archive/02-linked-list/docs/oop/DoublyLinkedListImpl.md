# oop/DoublyLinkedListImpl.java

양방향 연결 리스트 구현. 노드는 `prev`, `next` 두 포인터를 가진다. `removeLast`는 `tail.prev`로 O(1) 가능. `reverse`는 각 노드의 `prev`/`next`를 swap하면서 head/tail을 교체한다.
